import React from 'react'
import Card from './Card'
import { FiChevronLeft, FiChevronRight } from "react-icons/fi";
import { useState } from 'react';
import randomInteger from 'random-int';

const Testimonials = ({reviews}) => {
    const[activeIndex, setActiveIndex] = useState(0);
    function leftShiftHandler() {
        if (activeIndex===0) {
            console.log("Last left shift", activeIndex)
            setActiveIndex((reviews.length-1))
        } 
        else {
            let newInd = activeIndex-1;
            setActiveIndex(newInd)
        }
    }

    function rightShiftHandler() {
        if (activeIndex===(reviews.length-1)) {
            console.log("Last right shift", activeIndex)
            setActiveIndex(0)
        } 
        else {
            let newInd = activeIndex+1;
            setActiveIndex(newInd)
        }
    }

    function surpriseHandler() {
        let rndInt= Math.floor(Math.random() * ((reviews.length-1) - 0 + 1)) + 0;
        console.log("Random Integer is", rndInt);
        for (let index = 0; index <= rndInt; index++) {
            setActiveIndex(index)
            
        }
    }
  return (
    <div className='w-[85vw] md:w-[700px] bg-white flex flex-col justify-center items-center
    mt-10 p-10 transition-all duration-700 hover:shadow-xl rounded-md'>
      <Card review={reviews[activeIndex]}/>

      <div className='flex text-3xl mt-10 gap-3 text-violet-400 font-bold mx-auto'>
        <button 
        onClick={leftShiftHandler}
        className='cursor-pointer hover:text-violet-500 '>
            <FiChevronLeft/>
        </button>
        <button 
        onClick={rightShiftHandler}
        className='cursor-pointer hover:text-violet-500 '>
            <FiChevronRight/>
        </button>
      </div>

      <div className='mt-6'>
        <button
        onClick={surpriseHandler}
         className='bg-violet-400 hover:bg-violet-500 transition-all duration-200
        cursor-pointer px-10 py-2 rounded-md font-bold text-white text-lg'>
            Surprise Me
        </button>
      </div>
    </div>
  )
}

export default Testimonials
